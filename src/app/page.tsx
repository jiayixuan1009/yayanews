...
export default function HomePage() {
  ...
  const flashStream = getFlashNews(12);
  ...
  const flashMaxId = getFlashMaxId();
  const articleMaxId = getPublishedArticleMaxId();
      ...
      <section className="border-b border-[#ddd5ca] bg-[#fbf8f4]">
        <div className="container-main py-4">
          <div className="mb-3 flex flex-wrap items-baseline justify-between gap-2">
            ...
            <Link href="/news" className="font-label text-xs font-semibold uppercase tracking-[0.14em] text-[#1d5c4f] hover:text-[#143d33]">
              资讯总览
            </Link>
          </div>
        ...
        <div className="grid gap-8 lg:grid-cols-12 lg:gap-10 xl:gap-12">
              ...
              <section className="border border-[#ddd5ca] bg-white px-5 py-6 sm:px-7">
                ...
                <div className="mt-5 grid gap-6 md:grid-cols-3">
                  {spotlightArticles.slice(0, 3).map(item => (
                    <Link key={item.id} href={`/article/${item.slug}`} className="group block border-t border-[#e9e2d6] pt-4 md:border-t-0 md:pt-0">
          ...
          <aside className="space-y-5 lg:col-span-4 lg:space-y-6">
            ...
            <RightRailPanel title="新手指南" accent>
              ...
              <Link href="/guide" className="mt-4 inline-block font-label text-xs font-semibold uppercase tracking-[0.14em] text-[#1d5c4f] hover:text-[#143d33]">
                进入指南 →
              </Link>
            </RightRailPanel>
          </aside>
