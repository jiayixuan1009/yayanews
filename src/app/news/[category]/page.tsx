...
export default function CategoryPage({
  ...
  const articles = getPublishedArticles(36, 0, params.category, undefined, articleType);
  ...
  return (
    <div className="container-main py-6 sm:py-8 lg:py-10">
      ...
      <div className="mb-6 flex flex-wrap items-center gap-2 border-b border-[#ddd5ca] pb-4 lg:mb-8">
        <Link href="/news" className="border border-[#ddd5ca] bg-white px-3 py-1.5 text-[11px] uppercase tracking-[0.18em] text-[#667067] hover:text-[#14261f]">
          All desks
        </Link>
        {categories.map(c => (
          <Link
                ...
                <section className="grid gap-6 border-b border-[#ddd5ca] pb-10 lg:grid-cols-[minmax(0,1.25fr)_280px]">
                  ...
                  <div className="space-y-4">
                    {secondary.map(item => {
                      ...
                      return (
                        <Link key={item.id} href={`/article/${item.slug}`} className="group grid gap-3 border-b border-[#ddd5ca] pb-4 last:border-b-0 last:pb-0" style={{ gridTemplateColumns: '98px minmax(0,1fr)' }}>
