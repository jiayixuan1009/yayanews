...
export default function ArticlePage({ params }: { params: { slug: string } }) {
  ...
  const sameCategory =
    article.category_slug != null
      ? getPublishedArticles(8, 0, article.category_slug).filter(a => a.id !== article.id)
      : [];
        ...
        <article className="lg:col-span-8 xl:pr-2">
          <div className="mx-auto max-w-measure-wide">
            <nav className="yn-meta mb-5 flex flex-wrap gap-x-2 gap-y-1">
              <Link href="/" className="yn-link">
                首页
              </Link>
              <span aria-hidden>/</span>
              <Link href="/news" className="yn-link">
                资讯
              </Link>
              {article.category_name && article.category_slug ? (
                <>
                  <span aria-hidden>/</span>
                  <Link href={`/news/${article.category_slug}`} className="yn-link">
              ...
              {(prev || next) && (
                <section>
                  ...
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    {prev ? (
                      <Link href={`/article/${prev.slug}`} className="yn-panel group p-4 hover:border-[#bfb4a5]">
                    ...
                    {next ? (
                      <Link href={`/article/${next.slug}`} className="yn-panel group p-4 hover:border-[#bfb4a5] sm:text-right">
                  ...
                  <div className="flex items-start justify-between gap-4 border-b border-[#e5ddd2] pb-3">
                    ...
                    <dd>
                      {article.category_slug ? (
                        <Link href={`/news/${article.category_slug}`} className="yn-link text-right">
                          {article.category_name}
                        </Link>
                      ) : (
