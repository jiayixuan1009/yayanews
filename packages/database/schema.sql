...
CREATE INDEX IF NOT EXISTS idx_loop_runs_started_at ON loop_runs(started_at DESC);
CREATE INDEX IF NOT EXISTS idx_seo_feedback_latest ON seo_feedback_snapshots(source, imported_at DESC);
CREATE INDEX IF NOT EXISTS idx_seo_feedback_dimension_metrics ON seo_feedback_snapshots(dimension, impressions DESC, clicks DESC);
CREATE INDEX IF NOT EXISTS idx_content_opportunities_status_priority ON content_opportunities(status, priority DESC, score DESC, last_seen_at DESC);
CREATE INDEX IF NOT EXISTS idx_content_opportunities_type ON content_opportunities(opportunity_type, status);
CREATE INDEX IF NOT EXISTS idx_loop_actions_status_type ON loop_actions(status, action_type, risk_level);
CREATE INDEX IF NOT EXISTS idx_loop_actions_target ON loop_actions(target_kind, target_id);
CREATE UNIQUE INDEX IF NOT EXISTS loop_feedback_events_unique_row ON loop_feedback_events(source, import_batch, event_type, dedupe_key);
CREATE INDEX IF NOT EXISTS idx_loop_feedback_source_latest ON loop_feedback_events(source, imported_at DESC);
CREATE INDEX IF NOT EXISTS idx_loop_feedback_entity ON loop_feedback_events(entity_kind, entity_value, event_type);
CREATE INDEX IF NOT EXISTS idx_loop_feedback_metric ON loop_feedback_events(metric_name, metric_value DESC);
...
CREATE INDEX IF NOT EXISTS idx_articles_slug_indexable ON articles(slug) WHERE status = 'published' AND audit_status = 'approved' AND deleted_at IS NULL AND is_indexable = TRUE;
...
CREATE INDEX IF NOT EXISTS idx_articles_category_feed ON articles(category_id, lang, published_at DESC, id) WHERE status = 'published' AND audit_status = 'approved' AND deleted_at IS NULL AND is_indexable = TRUE;
CREATE INDEX IF NOT EXISTS idx_articles_topic_feed ON articles(topic_id, published_at DESC, id) WHERE status = 'published' AND audit_status = 'approved' AND deleted_at IS NULL AND is_indexable = TRUE;
CREATE INDEX IF NOT EXISTS idx_articles_published_id_feed ON articles(published_at DESC, id) WHERE status = 'published' AND audit_status = 'approved' AND deleted_at IS NULL AND is_indexable = TRUE;
CREATE INDEX IF NOT EXISTS idx_article_tags_article_tag ON article_tags(article_id, tag_id);
CREATE INDEX IF NOT EXISTS idx_articles_lang_type_feed ON articles(lang, article_type, published_at DESC, id) WHERE status = 'published' AND audit_status = 'approved' AND deleted_at IS NULL AND is_indexable = TRUE;
CREATE INDEX IF NOT EXISTS idx_flash_news_lang_published ON flash_news(lang, published_at DESC, id);
