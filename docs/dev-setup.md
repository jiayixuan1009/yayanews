...
```
如果目标环境无此插件（比如某些低版本的 Windows PostgreSQL 或云数据库），`apps/pipeline/` 的增量相似度去重将直接宕机。

> **注意：** 在 Windows 上如果遇到没有 `pgvector` DLL 的情况，需要在 `schema.sql` 中将该语句注释掉，但这这意味着你在本地不能运行 Pipeline 的全量嵌入功能。

## 6. PM2 工作流与隐藏环境变量
关于 `ecosystem.config.cjs`，存在一个默认**不加载** Python Pipeline 以防挤占本地开发资源的机制：

- `ENABLE_PYTHON_WORKERS=true`：如果需在 PM2 中启动爬虫/预处理节点，必须在 `.env` 加入此项指令。这只适用于 VPS 生产。
- `PYTHON_BIN`：指定运行 pipeline 的 Python 执行路径（建议配合本地 \`venv\` 或全局 \`python3\`）。

如果遇到「我的 Web 后台起了但新闻没采集」的问题，请检查该环境变量！
