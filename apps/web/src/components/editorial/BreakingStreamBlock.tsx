...
function getFilterTags(lang: string): { label: string; match: (name: string) => boolean }[] {
  ...
  if (lang === 'en') {
    return [
      { label: 'US Stocks',   match: n => norm(n).includes('美股') },
      { label: 'HK & APAC',   match: n => norm(n).includes('港股') || norm(n).includes('亚太') },
      { label: 'Crypto', match: n => norm(n).includes('加密') || norm(n).toLowerCase().includes('crypto') || norm(n).includes('比特') },
      { label: 'Derivatives', match: n => norm(n).includes('衍生') || norm(n).includes('期货') || norm(n).includes('期权') || norm(n).includes('宏观') },
    ];
  }
  return [
    { label: '美股',   match: n => norm(n).includes('美股') },
    { label: '港股',   match: n => norm(n).includes('港股') || norm(n).includes('亚太') },
    { label: '加密货币', match: n => norm(n).includes('加密') || norm(n).toLowerCase().includes('crypto') || norm(n).includes('比特') },
    { label: '衍生品', match: n => norm(n).includes('衍生') || norm(n).includes('期货') || norm(n).includes('期权') || norm(n).includes('宏观') },
  ];
}
...
function getCategoryBadgeLight(name?: string | null) {
  const n = name == null ? '' : String(name);
  if (!n) return 'bg-[#f4ebe1] text-[#7c837d] border-[#ece4d8]';
  if (n.includes('美股')) return 'bg-blue-50 text-blue-600 border-blue-200';
  if (n.includes('加密') || n.includes('比特币') || n.toLowerCase().includes('crypto')) return 'bg-amber-50 text-amber-700 border-amber-200';
  if (n.includes('港股') || n.includes('亚太')) return 'bg-rose-50 text-rose-600 border-rose-200';
  if (n.includes('衍生') || n.includes('宏观')) return 'bg-emerald-50 text-emerald-700 border-emerald-200';
...
function translateCategoryName(name: string, lang: string) {
  ...
  if (n.includes('美股')) return 'US Stocks';
  if (n.includes('港股') || n.includes('亚太')) return 'HK & APAC';
  if (n.includes('加密') || n.toLowerCase().includes('crypto')) return 'Crypto';
  if (n.includes('衍生') || n.includes('期货') || n.includes('宏观')) return 'Derivatives';
  if (n.includes('综合') || n.includes('其他')) return 'General';
  return n;
}
...
export default function BreakingStreamBlock({
  ...
  const visibleItems = selectedTags.size === 0
    ...
    : allItems.filter(item => {
        const name = String(item.category_name ?? '');
  ...
  return (
    <section id="breaking-stream-block" className={`yn-panel flex flex-col p-4 sm:p-5 ${className}`}>
