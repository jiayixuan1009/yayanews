  ...
  return (
    <div className="container-main py-12 md:py-20 lg:max-w-3xl mx-auto">
      ...
      <div className="space-y-12 text-[#14261f]">
        <section>
          ...
          <div className="space-y-4 text-sm leading-7 text-[#4a5250]">
            ...
            <div className="border-l-2 border-[#cfa840] pl-4">
              ...
              <p>{isZh
                ? '长篇解读类稿件，必须包含核心判断、支持依据、反方观点及风险提示。作者需具备相关领域专业背景。'
                : 'Long-form analysis requiring a core thesis, supporting evidence, counter-arguments and risk disclosures. Authors must have relevant domain expertise.'}</p>
            </div>
          </div>
        </section>
        ...
        <section>
          ...
          <ul className="space-y-3 text-sm leading-7 text-[#4a5250]">
            {(isZh ? [
              '所有新闻稿件优先引用可查阅的一手资料，包括官方公告、监管文件、财报原文等。',
              '二手来源或媒体报道须注明原始机构，并在正文体现其来源属性。',
              '观点类内容须明确标注为分析或评论，与事实性报道区分呈现。',
              'AI 辅助撰写的稿件须经人工编辑核实关键数据及事实后方可发布。',
              '涉及价格、链上数据或统计数据的稿件，应尽量标明数据截止时间。',
            ] : [
              'All articles should cite primary sources where available, including official announcements, regulatory filings and earnings releases.',
              'Secondary sources or media reports must credit the originating institution and show source context in the article.',
              'Opinion and analysis pieces must be clearly labeled and separated from factual reporting.',
              'AI-assisted content must be reviewed and fact-checked by human editors before publication.',
              'Articles citing price data, on-chain data or statistics should specify the data cutoff date when possible.',
            ]).map((item, i) => (
        ...
        <section>
          ...
          <p className="text-sm leading-7 text-[#4a5250]">
            {isZh
              ? `${SITE_NAME_ZH} 的编辑内容与商业合作保持分离。广告主、合作伙伴及关联公司不得干预新闻报道的角度、内容或结论。编辑团队保留对任何内容的最终决定权。`
              : `${SITE_NAME_EN}'s editorial content is separated from commercial partnerships. Advertisers, partners and affiliates have no influence over news coverage angle or conclusions. The editorial team retains final decision-making authority over all content.`}
          </p>
        </section>
        ...
        <section>
          ...
          <p className="text-sm leading-7 text-[#4a5250]">
            {isZh
              ? '发现事实性错误后，我们将在原文中进行更正，并在影响核心含义时保留更正说明。重大错误将以更明显方式披露。读者可以通过联系页面提交更正请求。'
              : 'When factual errors are identified, we correct the original article and add a visible note when the change affects the meaning. Significant errors receive more prominent disclosure. Readers can submit correction requests through our contact page.'}
          </p>
          <div className="mt-4">
            <LocalizedLink
              href="/corrections"
            ...
            >
              {isZh ? '查看更正政策' : 'View corrections policy'}
            </LocalizedLink>
          </div>
        </section>
        ...
        <section className="rounded-xl border border-amber-200 bg-amber-50 p-6">
          ...
          <p className="text-sm leading-7 text-amber-800">
            {isZh
              ? `${SITE_NAME_ZH} 发布的所有内容仅供参考，不构成投资建议、买卖推荐或专业金融意见。投资者应自行评估风险，并在必要时咨询持牌金融顾问。`
              : `All content published by ${SITE_NAME_EN} is for informational purposes only and does not constitute investment advice, a solicitation to buy or sell, or professional financial advice. Investors should conduct their own due diligence before making investment decisions.`}
          </p>
        </section>
      </div>
      ...
      <div className="mt-12 border-t border-[#e7dfd2] pt-8 text-xs text-[#667067]">
        {isZh
          ? `本政策最后更新于 ${new Date().toISOString().slice(0, 10)}。如有疑问，请联系编辑团队。`
