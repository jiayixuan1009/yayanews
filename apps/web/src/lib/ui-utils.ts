...
function titleToSlug(title: string): string {
  if (typeof title !== 'string') title = title == null ? '' : String(title);
  return title
 ...
 */
export function encodeFlashSlug(flash: {
  id?: unknown;
  title?: unknown;
  published_at?: unknown;
  created_at?: unknown;
}): string {
  const idNum = Number(flash?.id);
  const safeId = Number.isFinite(idNum) ? idNum : 0;
  const rawTitle = flash?.title;
  const titleStr = typeof rawTitle === 'string' ? rawTitle : rawTitle != null ? String(rawTitle) : '';
  if (titleStr.trim()) {
    const slug = titleToSlug(titleStr);
    if (slug) return `${slug}-${safeId}`;
  }
  // Legacy fallback
  const fallback = '0000000000';
  const dtStr = String(flash?.published_at ?? flash?.created_at ?? '');
  let cleaned = dtStr.replace(/[-T:Z.\s]/g, '');
  if (cleaned.length < 10) cleaned = fallback;
  const yyyymmddhh = cleaned.slice(0, 10);
  const paddedId = String(safeId).padStart(4, '0');
